...
inherit check-reqs cmake cuda flag-o-matic llvm-r1 pax-utils python-single-r1 toolchain-funcs virtualx xdg-utils
...
if [[ ${PV} = *9999* ]] ; then
	...
	# NOTE: Doesn't exist yet, commented out for now
	# SRC_URI+="
	# 	test? ( https://dev.gentoo.org/~sam/distfiles/${CATEGORY}/${PN}/${PN}-${TEST_TARBALL_VERSION}-tests.tar.xz )
	# "
...
RDEPEND="${PYTHON_DEPS}
	...
	osl? ( media-libs/osl:=[${LLVM_USEDEP}] )
...
PATCHES=(
	...
	"${FILESDIR}/${PN}-4.0.2-osl-1.13-support.patch"
)
...
src_configure() {
	...
	local mycmakeargs=(
		...
		-DWITH_TBB=$(usex tbb)
		$(usex osl "-DLLVM_ROOT_DIR=$(get_llvm_prefix)")
